...
Supports Anthropic (Claude) and OpenAI-compatible (DeepSeek) backends.
Configure via LLM_BACKEND env var: 'anthropic' or 'openai'.
"""
...
import app.tools.moderation    # noqa: F401


def _create_llm_client():
    """Create the appropriate LLM client based on backend configuration."""
...
class AgentEngine:
    ...
    def __init__(self, memory_db: MemoryDatabase) -> None:
        ...
        self._runner = AgentRunner(tool_registry)
    ...
    async def chat(self, ctx: AgentContext) -> AgentResponse:
        ...
        tokens_in = usage.input_tokens if usage and hasattr(usage, 'input_tokens') else 0
        tokens_out = usage.output_tokens if usage and hasattr(usage, 'output_tokens') else 0
    ...
    async def _write_audit(
        self, ctx: AgentContext, reply: str, tool_calls: list, usage, cost: float, start_time: float,
    ) -> None:
        try:
            ...
            duration_ms = int((time.monotonic() - start_time) * 1000)
            model = getattr(usage, 'model', settings.anthropic_default_model) if usage else settings.anthropic_default_model
            ...
            await self._memory.execute(
                """INSERT INTO audit_events
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (event_id, datetime.now().isoformat(), "chat", ctx.user_id, ctx.session_id,
                 ctx.conversation_id, "chat", "conversation", "success", duration_ms,
                 model,
                 usage.input_tokens if usage and hasattr(usage, 'input_tokens') else 0,
                 usage.output_tokens if usage and hasattr(usage, 'output_tokens') else 0,
                 0, round(cost, 6), tool_calls_json),
            )
            await self._memory.commit()
        except Exception as e:
