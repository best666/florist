...
<template>
  <view v-if="visible"
    ...
    <view class="flex items-center gap-3">
      <view
        class="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-surface)] text-lg shadow-[0_8rpx_18rpx_rgba(148,163,184,0.10)]">
        🪴
      </view>
      <view class="min-w-0 flex-1">
        <text class="block text-sm font-800 text-app-ink">刚需补给推荐：{{ featuredProduct.title }}</text>
        <text class="mt-1 block truncate text-2xs text-app-muted">静态弱广告，仅做养花商品导览，无弹窗、无强跳、无开屏广告</text>
      </view>
      <button class="h-9 rounded-full border-none bg-[#EEF3E9] px-4 text-2xs font-800 text-app-ink"
