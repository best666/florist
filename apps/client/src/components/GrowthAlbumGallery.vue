...
<template>
  <view class="grid grid-cols-3 gap-3">
    <view class="rounded-[28rpx] bg-[var(--color-surface)]/88 px-4 py-4 shadow-[0_14rpx_32rpx_rgba(148,163,184,0.12)] dark:bg-slate-900">
      <text class="block text-2xs text-app-muted/70 dark:text-app-muted">照片数量</text>
      <text class="mt-1 block text-xl font-800 text-app-ink dark:text-slate-100">{{ albumItems.length }}</text>
    </view>
    <view class="rounded-[28rpx] bg-[var(--color-surface)]/88 px-4 py-4 shadow-[0_14rpx_32rpx_rgba(148,163,184,0.12)] dark:bg-slate-900">
    ...
    <view class="rounded-[28rpx] bg-[var(--color-surface)]/88 px-4 py-4 shadow-[0_14rpx_32rpx_rgba(148,163,184,0.12)] dark:bg-slate-900">
      ...
      <text class="mt-1 block text-sm font-800 leading-6 text-app-ink dark:text-slate-100">{{ latestUpdatedText
        }}</text>
    </view>
  </view>
  ...
  <view class="card-soft rounded-[32rpx] transition-all duration-300 dark:bg-slate-900">
    ...
    <view v-if="albumItems.length > 0" class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
      <view v-for="item in albumItems" :key="item.id"
        ...
        <AppImage class="h-[220rpx] w-full rounded-[22rpx] bg-[var(--color-surface)] object-cover dark:bg-slate-900"
        ...
        <text class="mt-2 block text-sm leading-6 text-app-muted dark:text-slate-300">{{ item.subtitle }}</text>
