...
<template>
  <view class="card-soft rounded-[32rpx] dark:bg-slate-900">
    ...
    <view v-if="props.advice"
      class="mt-4 flex flex-col gap-4 rounded-[28rpx] bg-linear-to-br from-[#FFF8F0] via-white to-[#F5FCF8] p-4 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800">
      <view>
      ...
      <view class="rounded-[24rpx] bg-white/88 px-4 py-4 dark:bg-slate-900">
        ...
        <text class="mt-1 block text-sm leading-6 text-slate-700 dark:text-slate-200">
      ...
      <view class="grid grid-cols-2 gap-3">
        <view class="rounded-[24rpx] bg-white/88 px-4 py-4 dark:bg-slate-900">
          <text class="block text-2xs text-slate-400 dark:text-slate-500">今天优先做</text>
          <view class="mt-2 flex flex-col gap-2">
            <text v-for="item in props.advice.focusActions" :key="item"
        ...
        <view class="rounded-[24rpx] bg-white/88 px-4 py-4 dark:bg-slate-900">
          <text class="block text-2xs text-slate-400 dark:text-slate-500">今天先别急着做</text>
          <view class="mt-2 flex flex-col gap-2">
            <text v-for="item in props.advice.forbiddenActions" :key="item"
              class="text-sm leading-6 text-slate-700 dark:text-slate-200">
              {{ item }}
            </text>
          </view>
      ...
      <view class="rounded-[24rpx] bg-white/88 px-4 py-4 dark:bg-slate-900">
        <text class="block text-2xs text-slate-400 dark:text-slate-500">极端天气禁忌提醒</text>
        <view class="mt-2 flex flex-col gap-3">
          <view v-for="alert in props.advice.extremeWeatherAlerts" :key="alert.type"
            class="rounded-[18rpx] bg-app-ivory px-3 py-3 dark:bg-slate-800">
            <text class="block text-sm font-700 text-slate-800 dark:text-slate-100">{{ alert.title }}</text>
            <text class="mt-1 block text-sm leading-6 text-slate-500 dark:text-slate-300">{{ alert.description }}</text>
          </view>
          <text v-if="props.advice.extremeWeatherAlerts.length === 0"
            class="text-sm leading-6 text-slate-500 dark:text-slate-300">
            今天天气不算极端，可以按稳定节奏照顾它，不必太紧张。
          </text>
        </view>
      </view>

      <view v-if="props.message"
    ...
    <view v-else
      class="mt-4 rounded-[28rpx] bg-app-ivory/90 px-4 py-4 text-sm leading-6 text-slate-500 dark:bg-slate-800 dark:text-slate-300">
      {{ props.message || (props.disabled
        ? '当前离线，AI 建议先暂停一下。你可以先看盆土、叶片和今天的天气提示。'
        : '选中一盆植物后，我会按天气、摆放位置和历史养护记录，给出一份更具体的建议。') }}
    </view>

    <view class="mt-4">
      <SubmitBtn text="重新问问 AI" variant="mint" :disabled="props.loading || !props.flower" @click="emit('refresh')" />
    </view>
  </view>
</template>
