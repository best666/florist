      ...
      <scroll-view scroll-y class="max-h-[68vh] pr-1">
        <view class="flex flex-col gap-4 pb-4">
          ...
          <view class="rounded-[28rpx] bg-app-ivory/90 p-4 dark:bg-slate-800">
            ...
            <view class="mt-3 flex flex-wrap gap-2">
              <button
                v-for="option in RECORD_COOLDOWN_PRESET_OPTIONS"
                :key="option.value"
                ...
                hover-class="opacity-92"
                @tap="formState.cooldownMinutes = option.value"
              >
                {{ option.label }}
              </button>
            </view>
          ...
          <view class="rounded-[28rpx] bg-app-ivory/90 p-4 dark:bg-slate-800">
            ...
            <view class="mt-3 grid grid-cols-3 gap-3">
              ...
              <button
                v-if="formState.images.length < 4"
      ...
      <view class="mt-4 flex gap-3">
        ...
        <view class="flex-1">
          <SubmitBtn
            text="完成打卡"
            loading-text="保存中..."
            :loading="props.submitting || isUploadingImages"
            variant="mint"
            @click="handleSubmit"
          />
        </view>
